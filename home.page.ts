...
export class HomePage {
  ...
  async displayOnChart(mode) {
    ...
    if (mode == 'search') {await this.myServiceProvider.getOpeningStatFromServiceProvider(this.openingChoice, this.chartData);}
    else if (mode == 'White' || mode == 'Black') {await this.myServiceProvider.getTopFourOpeningFromServiceProvider(mode, this.chartData)}
      
    this.doughnutChartMethod(this.chartData[0], this.chartData[1]); 
